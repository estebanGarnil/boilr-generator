...
class FileGenerator:
    ...
    def copy_sources(
        ...
        for module in project.ordered_modules():
            for index, source in enumerate(
                ...
                field_path = (
                    f"modules.{module.key}.sources.copy[{index}].from"
                )
    ...
    def _copy_source(
        ...
        *,
        ...
        """Copy one source using its configured collision strategy."""
        if not source_path.exists():
            raise SourceNotFoundError(
                ...
                suggestion=(
                    "Check the source path declared in the module manifest "
                    "and ensure the referenced file or directory is packaged."
                ),
            )
    ...
    def render_sources(
        ...
        """Render every template declared by the resolved modules."""
        output_path = Path(output_path)
        ...
        for module in project.ordered_modules():
            for index, source in enumerate(
                ...
                destination_path = output_path / source.to
                field_path = (
                    f"modules.{module.key}.sources.render[{index}].from"
                )
    ...
    def _render_template(
        ...
        *,
        ...
        """Render one template using strict Jinja variables."""
        if not template_path.is_file():
            raise SourceNotFoundError(
                ...
                suggestion=(
                    "Check the template path declared in the module manifest "
                    "and ensure the template is packaged."
                ),
            )
        ...
        try:
            template = JINJA_ENVIRONMENT.from_string(template_content)
            rendered_content = template.render(**context)
        except TemplateError as error:
            raise TemplateRenderError(
                f"Unable to render template '{template_path}': {error}",
                module_key=module_key,
                field_path=field_path,
                context={
                    "target": "file",
